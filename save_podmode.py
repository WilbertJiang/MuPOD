...
data_filename = './buidling_blk_'+ str(N_index)+'/corelationmatrix.txt' 
...
eigensolver.solve()
header = './buidling_blk_'+ str(N_index)+'/eigenvalue_r'                               
...
#for i in range (0,Train_steps):
    ...
    podmode.append(a)
    pod_filename = './buidling_blk_'+ str(N_index)+'/podmode_fenics.txt'
    ...
    mode_file_name = "./buidling_blk_"+ str(N_index)+"/podmode/mode_" + str(i) + "h5"
    mode_file = HDF5File(mesh.mpi_comm(), mode_file_name, "w")
    mode_file.write(a,"solution")
    mode_file.close()

header = './buidling_blk_'+ str(N_index)+'/C_matrix'
...
header ='./buidling_blk_'+ str(N_index)+ '/G_matrix'
G_matrix_file_name = header + '.txt'
G_matrix_file = open(G_matrix_file_name,'w')
G_Matrix = np.zeros((N_mode,N_mode))
for i in range (0,N_mode):
        for j in range (0,N_mode):
                G_Matrix[i][j] = assemble(kappa*dot(grad(podmode[i]),grad(podmode[j]))*dx)
