...
if N_index ==1 or N_index ==3 or N_index ==7 or N_index ==9 or N_index ==11:
    ...
    y = np.linspace(flp[N_index-1][3] + tol, flp[N_index-1][3]+flp[N_index-1][1]- tol, 100)
#points = [(0.00239 + 0.00122, y_,h) for y_ in y]  # 2D points
    points = [(0.00239 + 0.00122, y_,h) for y_ in y]  # 2D points

    p_line = np.array([solution[p_i](point) for point in points])

    header_data = 'solution_plot/y_block'+str(N_index)
    ...
    header_data = 'solution_plot/T_Y_block'+str(N_index)+'_'+str(N_index2)+'mode'
    data_file_name = header_data + '.txt'
    data_file = open(data_file_name,'w')
    for i in range(0,len(y)):
        data_file.write('%.16g\n' % (p_line[i]))
...
if N_index ==2 or N_index ==9 or N_index ==4 or N_index ==10 or N_index ==6:
    tol = 1e-14
    y = np.linspace(flp[N_index-1][2] + tol, flp[N_index-1][2]+flp[N_index-1][0]- tol, 100)
    points = [(y_,0.009205,h) for y_ in y]
    p_line = np.array([solution[p_i](point) for point in points])
    header_data = 'solution_plot/x_block'+str(N_index)
    data_file_name = header_data + '.txt'
    data_file = open(data_file_name,'w')
    for i in range(0,len(y)):
        data_file.write('%.16g\n' % (y[i]))
    data_file.close()
    header_data = 'solution_plot/T_X_block'+str(N_index)+'_'+str(N_index2)+'mode'
    data_file_name = header_data + '.txt'
    data_file = open(data_file_name,'w')
    for i in range(0,len(y)):
        data_file.write('%.16g\n' % (p_line[i]))
