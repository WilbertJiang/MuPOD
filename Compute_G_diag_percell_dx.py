...
for n in range(0,N_mode): 
    podmode_load_file_name = "./buidling_blk_"+ str(N_index)+"/podmode/mode_" + str(n) + "h5"
...
"""
for n in range(0,N_mode):
    podmode_load_file_name = "./finite_difference_method/podmode_grad_X/mode_" + str(n) + "h5"
    podmode_file = HDF5File(mesh.mpi_comm(), podmode_load_file_name, "r")
    podmode_file.read(u, "solution")
    grad_x[n] = interpolate(u0,V)
    grad_x[n].assign(u)
    podmode_file.close()
for n in range(0,N_mode):
...
for i in range (0,N_mode):
    print(i)
    u_n_cell =interpolate(podmode[i],DG)
    for j in range (0,N_mode):
        ...
        for cell in cells(mesh):
            M = cell.midpoint()
            if M.x()< ll + 1.19e-4:
                G_BC_matrix[i][0+j*4]+= (-assemble_local(F,cell))
        F = dot(u_n_cell,grad(podmode[j])[0])*dx
        for cell in cells(mesh):
            M = cell.midpoint()
            if M.x()> lr - 1.19e-4:
                G_BC_matrix[i][1+j*4]+= (assemble_local(F,cell))
        F = dot(u_n_cell,grad(podmode[j])[1])*dx
        for cell in cells(mesh):
            M = cell.midpoint()
            if M.y()< wb + 1.19e-4:
                G_BC_matrix[i][2+j*4]+= (-assemble_local(F,cell))
        F = dot(u_n_cell,grad(podmode[j])[1])*dx
        for cell in cells(mesh):
            M = cell.midpoint()
            if M.y()> wt - 1.19e-4:
                G_BC_matrix[i][3+j*4]+= (assemble_local(F,cell))
##############save the G_matrix_BC_diag
header = './buidling_blk_'+ str(N_index)+'/G_matrix_BC_diag_block'+str(N_index)
...
for i in range (0,N_mode):
    for j in range (0,N_mode):
        if i <= j:
            G_BC_matrix_penalty[i][0+j*4] = assemble(inner(podmode[i],podmode[j])*ds(0))
            G_BC_matrix_penalty[i][1+j*4] = assemble(inner(podmode[i],podmode[j])*ds(1))
            G_BC_matrix_penalty[i][2+j*4] = assemble(inner(podmode[i],podmode[j])*ds(2))
            G_BC_matrix_penalty[i][3+j*4] = assemble(inner(podmode[i],podmode[j])*ds(3))
        else:
...
##############save the G_matrix_BC_diag
header ='./buidling_blk_'+ str(N_index)+ '/G_matrix_BC_diag_penalty_block'+str(N_index)
